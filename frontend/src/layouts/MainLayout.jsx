  ...
  return (
    <div className="min-h-screen ">
      <div className='flex justify-center w-full px-4 sm:px-6 lg:px-8'>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3  py-4 sm:px-6 lg:px-8 ">
              ...
              <>
                <Link to={dashboardPath} className="group">
                  <Button 
                    variant="secondary" 
                    size="sm" 
                    className="transition-all duration-300 transform hover:scale-105 hover:-translate-y-0.5 hover:shadow-lg"
                  >
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform group-hover:translate-y-1 group-hover:scale-105">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                      <p className="text-xs text-slate-400">Signed in as:</p>
                    </div>
                    <p className="text-sm text-white font-medium truncate transition-all duration-300 group-hover:text-yellow-400">{currentUser?.email}</p>
                  </div>
                </div>
              </>
            ) : (
              <>
                ...
                <Link to="/register" className="group">
                  <Button 
                    variant="" 
                    size="sm" 
                    className="bg-transparent text-white border border-white hover:bg-white hover:text-slate-950 hover:-translate-y-0.5"
                  >
                  Sign up
                  </Button>
                </Link>
      ...
      <footer className="border-t border-white/10 bg-black">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.1fr_0.7fr_0.7fr_0.9fr] lg:px-8">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              
              <div className="relative flex h-11 w-11 items-center justify-center">
                <ServiceHiveLogo className="h-10 w-10" />
              </div>
              <div>
                <p className="font-['Space_Grotesk'] text-lg font-bold text-white">ServiceHive</p>
                <p className="text-sm text-slate-400">Professional local services, reimagined.</p>
              </div>
            </div>
            <p className="max-w-md text-sm text-slate-400">
              A polished marketplace for discovering, booking, and managing trusted services with confidence.
            </p>
          </div>

          <div className="space-y-3">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-300">Platform</p>
            <Link to="/" className="block text-sm text-slate-400 transition hover:text-white">Home</Link>
            <Link to="/browse-services" className="block text-sm text-slate-400 transition hover:text-white">Browse Services</Link>
            <Link to="/about" className="block text-sm text-slate-400 transition hover:text-white">About</Link>
          </div>

          <div className="space-y-3">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-300">Account</p>
            <Link to="/login" className="block text-sm text-slate-400 transition hover:text-white">Login</Link>
            <Link to="/register" className="block text-sm text-slate-400 transition hover:text-white">Register</Link>
            <Link to={dashboardPath} className="block text-sm text-slate-400 transition hover:text-white">Workspace</Link>
          </div>

          <div className="space-y-4">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-300">Experience</p>
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4">
              <p className="text-base font-semibold text-white">Animated, image-rich UI</p>
              <p className="mt-2 text-sm text-slate-400">
                Category visuals, responsive surfaces, and role-focused dashboards now feel cohesive on every page.
              </p>
            </div>
          </div>
        </div>
