...
const BookingFormPage = () => {
  ...
  // Get service data from sessionStorage (passed from booking modal)
    ...
    const loadService = async () => {
      ...
      if (pendingBooking) {
        try {
          ...
          if (!cancelled) {
            setService({
              id: parsed.serviceId || id,
              title: parsed.serviceTitle || 'Service',
              provider: parsed.providerName || 'Service Provider',
              price: parsed.price || parsed.basePrice || 0,
              duration: parsed.duration || '2-3 hours',
      ...
      try {
        ...
        const response = await fetch(`${base}/services/${id}`)
        ...
        setService({
          ...
          provider: data.providerName || data.providerId?.name || 'Service Provider',
        ...
        if (!cancelled) {
          setServiceError(error.message || 'Could not load service')
        }
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bookingPayload = {
        ...
        status: 'pending',
        totalPrice: (service?.price || 0) + 13 // Add platform fee
      }
              ...
              <div className="space-y-2">
                ...
                <div className="border-t border-white/10 pt-2">
                  <div className="flex justify-between text-white font-semibold">
                    <span>Total</span>
                    <span className="text-xl gradient-text">RS ${service.price + 13}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  if (!service) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        <p className="text-gray-400">{serviceError || 'Service not found'}</p>
